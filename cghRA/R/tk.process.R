...
tk.process = function(
	...
	segmentAction <- function() {
		if(tcltk::tclvalue(segValue) == "1") {
			# Enable widgets
			tcltk::tkconfigure(cbsSpin, state="readonly")
			...
			tcltk::tclvalue(cbsSpinValue) <- "fast"
			tcltk::tclvalue(gapsValue) <- "1"
			tcltk::tclvalue(modelizeValue) <- "1"
		} else {
			# Disable widgets
			tcltk::tkconfigure(cbsSpin, state="disabled")
	...
	cbsSpinAction <- function() {
		profile <- tcltk::tclvalue(cbsSpinValue)
		if(profile == "custom") {
			tcltk::tkconfigure(cbsText, state="normal")
		} else {
			tcltk::tkconfigure(cbsText, state="normal")
	...
	tcltk::tkgrid.columnconfigure(localTopLevel, 1, weight=1)
		...
		tcltk::tkgrid.columnconfigure(preproFrame, 3, weight=1)
			...
			tcltk::tkgrid(probeFilesButton, column=1, row=1, padx=5, pady=5)
			tcltk::tkgrid(probeFilesLabel, column=2, columnspan=2, row=1, padx=5, pady=5, sticky="w")
			...
			parserSpin <- tcltk::ttkspinbox(parent=preproFrame, values=c("Agilent.probes", "custom.probes"), textvariable=parserValue, wrap=1, width=20, justify="center")
			tcltk::tkgrid(parserLabel, column=1, row=2, padx=5, pady=5)
			tcltk::tkgrid(parserSpin, column=2, columnspan=2, row=2, padx=5, pady=5, sticky="w")
			...
			tcltk::tkgrid(outputButton, column=1, row=3, padx=5, pady=5)
			tcltk::tkgrid(outputEntry, column=2, columnspan=2, row=3, padx=5, pady=5, sticky="we")
			...
			tcltk::tkgrid(designButton, column=1, row=4, padx=5, pady=5)
			tcltk::tkgrid(designEntry, column=2, columnspan=2, row=4, padx=5, pady=5, sticky="we")
			...
			tcltk::tkgrid(maskLabel, column=1, row=5, padx=5, pady=5)
			tcltk::tkgrid(maskCheck, column=2, row=5, padx=0, pady=5, sticky="w")
			tcltk::tkgrid(maskEntry, column=3, row=5, padx=5, pady=5, sticky="we")
			...
			replicateEntry <- tcltk::tkentry(parent=preproFrame, textvariable=replicateFunValue, width=15)
			tcltk::tkgrid(replicateLabel, column=1, row=6, padx=5, pady=5)
			tcltk::tkgrid(replicateCheck, column=2, row=6, padx=0, pady=5, sticky="w")
			tcltk::tkgrid(replicateEntry, column=3, row=6, padx=5, pady=5, sticky="we")
			...
			tcltk::tkgrid(wacaLabel, column=1, row=7, padx=5, pady=5)
			tcltk::tkgrid(wacaCheck, column=2, row=7, padx=0, pady=5, sticky="w")
			...
			tcltk::tkgrid(spatialLabel, column=1, row=8, padx=5, pady=5)
			tcltk::tkgrid(spatialCheck, column=2, row=8, padx=0, pady=5, sticky="w")
			...
			clusterEntry <- tcltk::tkentry(parent=preproFrame, textvariable=clusterValue)
			tcltk::tkgrid(clusterLabel, column=1, row=9, padx=5, pady=5)
			tcltk::tkgrid(clusterEntry, column=2, columnspan=2, row=9, padx=5, pady=5, sticky="we")
		...
		tcltk::tkgrid.columnconfigure(segFrame, 2, weight=1)
			...
			tcltk::tkgrid(segLabel, column=1, row=1, padx=5, pady=5)
			tcltk::tkgrid(segCheck, column=2, row=1, padx=0, pady=5, sticky="w")
			...
			tcltk::tkgrid(gapsLabel, column=1, row=2, padx=5, pady=5)
			tcltk::tkgrid(gapsCheck, column=2, row=2, padx=0, pady=5, sticky="w")
			...
			tcltk::tkgrid(cbsLabel, column=1, row=3, padx=5, pady=5, sticky="n")
			tcltk::tkgrid(cbsFrame, column=2, row=3, sticky="we")
			tcltk::tkgrid.columnconfigure(cbsFrame, 5, weight=1)
			
				# Profile spin
				cbsSpinValue <- tcltk::tclVar("fast")
				cbsSpin <- tcltk::ttkspinbox(parent=cbsFrame, values=c(process.default()$segmentArgs, "custom"), textvariable=cbsSpinValue, wrap=1, width=20, justify="center", command=cbsSpinAction, state="readonly")
				### cbsSpin <- tcltk::tkwidget(parent=cbsFrame, type="spinbox", values=c(process.default()$segmentArgs, "custom"), wrap=1, width=20, justify="center", command=cbsSpinAction)
				tcltk::tkgrid(cbsSpin, column=1, columnspan=2, row=1, padx=5, pady=c(5,0), sticky="w")
				
				# Details
				cbsScroll <- tcltk::tkscrollbar(parent=cbsFrame, repeatinterval=5, command=function(...) { tcltk::tkyview(cbsText,...) })
				tcltk::tkgrid(cbsScroll, column=2, row=2, padx=c(1, 5), pady=5, sticky="nsew")
				...
				tcltk::tkgrid(cbsText, column=1, row=2, padx=c(5, 1), pady=5, sticky="nsew")
		...
		tcltk::tkgrid.columnconfigure(modelFrame, 2, weight=1)
			...
			tcltk::tkgrid(modelizeLabel, column=1, row=1, padx=5, pady=5)
			tcltk::tkgrid(modelizeCheck, column=2, row=1, padx=0, pady=5, sticky="w")
			...
			tcltk::tkgrid(fittestLabel, column=1, row=2, padx=5, pady=5)
			tcltk::tkgrid(fittestSegmCheck, column=2, row=2, padx=0, pady=5, sticky="w")
			...
			tcltk::tkgrid(modelLabel, column=1, row=3, padx=5, pady=5)
			tcltk::tkgrid(modelEntry, column=2, row=3, padx=5, pady=5, sticky="we")
